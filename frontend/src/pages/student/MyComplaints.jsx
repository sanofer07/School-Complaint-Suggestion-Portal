...
import { STATUS_COLORS, PRIORITY_COLORS, CATEGORIES, STATUSES, formatDate } from '../../utils/helpers';
...
const MyComplaints = () => {
  ...
  const fetchComplaints = async () => {
    setLoading(true);
    try {
      const { data } = await complaintAPI.getMy({ ...filters, page, limit: 10 });
    ...
    <StudentLayout title="My Complaints">
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search by title or ticket ID" className="input-field pl-10"
            />
          </div>
      ...
      <div className="card">
        {loading ? (
          ...
          <EmptyState icon={<FiClipboard />} title="No complaints found" message="Try adjusting your filters or submit a new complaint." />
        ) : (
          <>
                  ...
                  {complaints.map((c) => (
                    <tr key={c._id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="py-3 pr-4">
                        <Link to={`/student/complaint/${c._id}`} className="text-primary-600 font-medium">View</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
